...
var tweets = [];
	...
	*/
	...
	function updateCanvas () {
		...
		//LOOP THROUGH THE TWEETS ARRAY AND UPDATE THE 'TWEETS' OBJECTS TO THEIR NEW LOCATION
		//SEE nyLatLng, call me and i can help
		
		
		/*
		//console.log(mapZoom);
		var rad = $(document).width()/5.5;
		var randomX = randomInt (point.x - rad, point.x + rad);
		var randomY = randomInt (point.y - rad, point.y + rad);
		//create loop that converts points to lat long then checks google to see if they are accurate, if so send them through to animate		
			
		var googleTest = overlay.getProjection().fromContainerPixelToLatLng(new google.maps.Point(randomX, randomY));
		console.log("latlng :" + googleTest);
		
		var geocoder = new google.maps.Geocoder();
		geocoder.geocode({"latLng":googleTest}, function(results,status){			
			console.log("status: " + status);
			if (status === 'OK') {
				createTweets(randomX, randomY);
			} 
		});				
		*/			
	...
	createTweetNodes.prototype = new google.maps.OverlayView();
	...
	*/
	...
	function createCanvasOverlay(canvasContainer) {     
		...
		createTweets = function(x, y) {
			for (var i = 0; i < tweetsLength; i++) {
				//push based on location lat and long
				tweets.push(new tweetCircle (x, y));
			}
		}
		...
		//draw shit
		var centerRingOpacity = [0.02, 0.8, 0.6, 0.4]; 
		...
		function animate() {	
			...
			if (point != undefined) {
					...
					context.beginPath();
						context.arc(point.x, point.y, myCanvas.width/3.9, 0, 2 * Math.PI, false);
					context.stroke();
				...
				context.save();
					context.translate(point.x, point.y);
		      		rotation += 0.1;
		      		context.rotate((Math.PI / 180) * rotation -2);
					context.beginPath();
						context.strokeStyle= "rgba(0, 230, 255," + centerRingOpacity[1] + ")";
						context.arc(0, 0,  myCanvas.width/3.9 - 6, 1.1 * Math.PI, 1.9 * Math.PI, false);
					context.stroke();
				context.restore();			
				...
				context.save();
					context.translate(point.x, point.y);
		      		rotation++;
		      		context.rotate((Math.PI / 180) * rotation * -1 - 10);
					context.beginPath();
						context.strokeStyle= "rgba(0, 230, 255," + centerRingOpacity[2] + ")";
						context.arc(0, 0,  myCanvas.width/3.9 - 12, 1.1 * Math.PI, 1.9 * Math.PI, false);
					context.stroke();
				context.restore();		
				...
				context.save();
					context.translate(point.x, point.y);
		      		rotation++;
		      		context.rotate((Math.PI / 180) * rotation * -1 - 5);
					context.beginPath();
						context.strokeStyle= "rgba(0, 230, 255," + centerRingOpacity[3] + ")";
						context.arc(0, 0,  myCanvas.width/3.9 - 18, 0 * Math.PI, 1.1 * Math.PI, false);
					context.stroke();
				context.restore();
